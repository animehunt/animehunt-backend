...
- **`utils/clientIp.js`** — shared `getClientIP(c, fallback)` helper, used consistently everywhere an IP is resolved (`middleware/firewall.js`, `middleware/dbSync.js`, `routes/publicSearch.js`, `routes/bannersPublic.js`, `routes/analytics.js`, `routes/ads.js`, `ai/playerEngine.js` ×3 call sites). Priority is **`x-forwarded-for` first, `CF-Connecting-IP` as fallback** — nginx is what actually sets `x-forwarded-for` on this deployment (`proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`), so it's authoritative whether or not you keep Cloudflare in front; `CF-Connecting-IP` still works as a fallback if you do. **This supersedes an earlier line in this same doc** that said the opposite order (`CF-Connecting-IP` first) — that was the original migration plan before `utils/clientIp.js` was written; the code and this corrected note are what's actually deployed.
...
1. ~~**`downloads.js` vs `bulk-upload.js`**~~ — **resolved**: both registered `POST /bulk-upload/download-links`. Compared the two implementations against the live schema — `bulk-upload.js`'s version targeted an older, simpler `download_links` design (columns `episode_id, url, host, quality` that don't exist on the current table) and would have failed on every call; `downloadsAdmin.js`'s version matches the current three-table `download_entries`/`download_host_entries`/`download_links` design and is the one actually mounted. Removed the broken duplicate from `bulk-upload.js`, left a comment there pointing to `downloadsAdmin.js` as the one real implementation.
