 ...
 * ════════════════════════════════════════════════════════
 * Download management router (Hono + Cloudflare D1) — PUBLIC ROUTES ONLY
 *
 * index.js mein mount karo:
 *   app.route("/api", downloads)   ← this file, public routes only
 *
 * ✅ FIX (security audit, dual-mount vulnerability): this file used to
 * contain BOTH public routes AND every admin CRUD route (hosts, download
 * entries, quick-add, bulk CSV upload, broken-link management) in one
 * Hono() instance. index.js mounted that same router object twice —
 * once publicly (app.route("/api", downloads), no auth) and once under
 * adminRoutes (adminRoutes.route("/", downloads), auth required).
 *
 ...
 * The result: every admin write route (POST/PUT/DELETE on hosts,
 * download_entries, download_host_entries, bulk-upload, etc.) was
 * reachable completely unauthenticated at /api/... — the exact same
 * problem already identified and fixed for player.js (see ISSUE-020 /
 * playerAdmin.js). This split applies that same fix here.
 *
 * All admin routes have moved to downloadsAdmin.js (mounted only under
...
/* ── HELPERS (shared by /go and /analytics above) ── */
