/* ================================================
   playerAdmin.js — Netflix-Style Player Settings (ADMIN WRITES)

   ✅ NEW FILE (audit ISSUE-020): split out of player.js. These two routes
   (POST /player, POST /player/reset) were previously in player.js, which
   was mounted ONLY publicly (app.route("/api", player) in index.js) — the
   entire admin route mount list never included player.js at all. That
   meant these write endpoints were reachable by anyone, unauthenticated,
   at /api/player and /api/player/reset — able to rewrite the site's whole
   player configuration, including the security settings covered in
   playerEngine.js (embed-lock, hotlink-block).

   This file is mounted under adminRoutes in index.js instead, so it picks
   up the requireAuth middleware every other admin route already has —
   the same fix pattern this codebase already uses correctly for
   downloads.js/ads.js (dual-mounted: public read routes in one file,
   admin write routes reachable only under /api/admin).

   Matches admin/player.html's own PLAY_API constant, which was already
   set to "/api/admin/player" — the frontend was written for this correct
   design from the start; only the backend mount was missing.
================================================ */
...
/* ================================================
   ...
   (identical to player.js's copy — table is shared, CREATE TABLE IF NOT
   EXISTS is idempotent so having it in both files is safe)
================================================ */
...
async function ensureRow(db) {
  try {
    ...
    console.error("playerAdmin ensureRow:", err)
  }
}

/* ================================================
   SYNC TO REPLICAS
   (identical to player.js's copy)
================================================ */
...
async function syncToReplicas(env, row) {
  ...
  if (env.TURSO_REPLICA_URL && env.TURSO_REPLICA_AUTH_TOKEN) {
    promises.push(fetch(`${tursoHttpUrl(env.TURSO_REPLICA_URL)}/v2/pipeline`, {
      method: "POST",
      headers: {
...
/* ================================================
   POST /player — Save (ADMIN ONLY — see file header)
================================================ */
...
app.post("/player", async (c) => {
  try {
    ...
    console.error("playerAdmin POST:", err)
    return c.json(failure(err.message), 500)
  }
})

/* ================================================
   POST /player/reset (ADMIN ONLY — see file header)
================================================ */
